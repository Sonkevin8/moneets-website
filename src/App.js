...
function AdminPanel({ content, setContent, customCss, setCustomCss, onExit }) {
  ...
  window.__adminPhotoState = { photos: easyDraft.backgroundPhotos, files: backgroundFiles, onChange: handlePhotoChange, onRemove: handlePhotoRemove, onMove: handlePhotoMove };
...
function BackgroundPhotoManager({ photos, files, onPhotoChange, onPhotoRemove, onPhotoMove }) {
  ...
  return <div className="photo-manager"><small className="photo-label">Carousel photos. Preview, replace, remove, or reorder them, then click Save Changes.</small>{Array.from({ length: 6 }, (_, index) => { const source = previews[index] || photos[index]; return <div className="photo-slot" key={index}>{source ? <img src={source} alt={`Carousel ${index + 1}`} /> : <span className="photo-empty">Empty slot</span>}<span className="photo-slot-label">Photo {index + 1}</span><input className="photo-picker" type="file" accept="image/*" onChange={event => event.target.files[0] && onPhotoChange(index, event.target.files[0])} />{(photos[index] || files[index]) && <div className="photo-slot-actions"><button className="photo-move" type="button" disabled={index === 0} onClick={() => onPhotoMove(index, -1)} aria-label={`Move photo ${index + 1} left`}>←</button><button className="photo-move" type="button" disabled={index >= photos.length - 1 && index >= files.length - 1} onClick={() => onPhotoMove(index, 1)} aria-label={`Move photo ${index + 1} right`}>→</button><button className="photo-remove" type="button" onClick={() => onPhotoRemove(index)}>Remove</button></div>}</div>; })}</div>;
}
