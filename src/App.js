...
function BackgroundCarousel({ photos, carouselSpeed }) {
  const [activePhoto, setActivePhoto] = useState(0);
  useEffect(() => {
    if (photos.length < 2) return undefined;
    const timer = window.setInterval(() => setActivePhoto(photo => (photo + 1) % photos.length), carouselSpeed * 1000);
    return () => window.clearInterval(timer);
  }, [photos.length, carouselSpeed]);
...
function AdminPanel({ content, setContent, customCss, setCustomCss, onExit }) {
  ...
  useEffect(() => {
    setEasyDraft(content);
    setThemeDraft(content.theme);
  ...
  const handlePhotoMove = (index, direction) => { const target = index + direction; if (target < 0 || target >= easyDraft.backgroundPhotos.length) return; setEasyDraft(current => ({ ...current, backgroundPhotos: current.backgroundPhotos.map((photo, photoIndex) => photoIndex === index ? current.backgroundPhotos[target] : photoIndex === target ? current.backgroundPhotos[index] : photo) })); setBackgroundFiles(current => current.map((file, fileIndex) => fileIndex === index ? current[index + direction] : fileIndex === target ? current[index] : file)); };
  window.__adminPhotoState = { photos: easyDraft.backgroundPhotos, files: backgroundFiles, speed: easyDraft.carouselSpeed, onSpeedChange: value => updateEasy('carouselSpeed', value), onChange: handlePhotoChange, onRemove: handlePhotoRemove, onMove: handlePhotoMove };
...
function EditorField({ label, value, onChange, area = false, photos, onPhotoChange, onPhotoRemove }) {
  ...
  return <label className="editor-field">{label}<Input value={value || ''} onChange={event => onChange(event.target.value)} />{(photos || label === 'Business name') && photoState && <BackgroundPhotoManager photos={photos || photoState.photos} files={photoState.files} speed={photoState.speed} onSpeedChange={photoState.onSpeedChange} onPhotoChange={onPhotoChange || photoState.onChange} onPhotoRemove={onPhotoRemove || photoState.onRemove} onPhotoMove={photoState.onMove} />}</label>;
}

function BackgroundPhotoManager({ photos, files, speed, onSpeedChange, onPhotoChange, onPhotoRemove, onPhotoMove }) {
  ...
  return <div className="photo-manager"><small className="photo-label">Carousel photos. Preview, replace, remove, or reorder them, then click Save Changes.</small><label className="carousel-speed">Seconds between photos<input type="number" min="1" max="60" value={speed} onChange={event => onSpeedChange(Math.max(1, Math.min(60, Number(event.target.value) || 1)))} /></label>{Array.from({ length: 6 }, (_, index) => { const source = previews[index] || photos[index]; return <div className="photo-slot" key={index}>{source ? <img src={source} alt={`Carousel ${index + 1}`} /> : <span className="photo-empty">Empty slot</span>}<span className="photo-slot-label">Photo {index + 1}</span><input className="photo-picker" type="file" accept="image/*" onChange={event => event.target.files[0] && onPhotoChange(index, event.target.files[0])} />{(photos[index] || files[index]) && <div className="photo-slot-actions"><button className="photo-move" type="button" disabled={index === 0} onClick={() => onPhotoMove(index, -1)} aria-label={`Move photo ${index + 1} left`}>←</button><button className="photo-move" type="button" disabled={index >= photos.length - 1 && index >= files.length - 1} onClick={() => onPhotoMove(index, 1)} aria-label={`Move photo ${index + 1} right`}>→</button><button className="photo-remove" type="button" onClick={() => onPhotoRemove(index)}>Remove</button></div>}</div>; })}</div>;
}
